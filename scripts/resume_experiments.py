...
def update_experiment_patch(
    previous_conf_path: str | Path, additional_n_iters: int | None = None
) -> dict:
    ...
    if config["on_policy"]:
        frames_per_batch = config["experiment_config"][
...
def main():
    ...
    for exp_folder in experiments_dir.iterdir():
        ...
        updated_conf = update_experiment_patch(
            conf_file, additional_n_iters=ADDITIONAL_N_ITERS
        )

        # Reload the experiment and patch the configuration to increase the iteration limit
        experiment = Experiment.reload_from_file(
            restore_file=str(latest_ckpt), experiment_patch=updated_conf
        )
        #
        print("-" * 80)
        print(
            f"Resuming experiment with seed={experiment.seed}, task={experiment.task_name}, algorithm={experiment.algorithm_name}"
        )
        print("-" * 80)
