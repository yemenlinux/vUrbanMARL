...
class ExperimentResult:
    ...
    def __init__(self, path: Path) -> None:
        ...
        self.csv_files = (
            list(self.scalars_dir.glob("*.csv")) if self.scalars_dir.exists() else []
        )
...
def load_metric_over_seeds(
    ...
    for exp in experiments:
        ...
        if df is not None and not df.empty:
            ...
            step_col = (
                "Step" if "Step" in df.columns else df.columns[0]
            )  # assume first is step
...
def aggregate_by_algorithm(
    df: pd.DataFrame, group_cols: List[str] = ["step", "algorithm"]
) -> pd.DataFrame:
...
def aggregate_runs(metric_dfs):
    ...
    result = pd.DataFrame({"step": steps_sorted, "mean": mean_vals, "std": std_vals})
    return result
...
def aggregate_runs_by_envs(metric_dfs):
    ...
    result = pd.DataFrame({"E": env_sorted, "mean": mean_vals, "std": std_vals})
    return result


def plot_metric_by_envs(
    metric_name, aggregated_dict, output_dir, algo_names=None, pdf=False
):
    """
...
def plot_metric(metric_name, aggregated_dict, output_dir, algo_names=None, pdf=False):
    """
    Plot aggregated metric for each algorithm (from aggregated_dict) and save figure.
    """
    if not aggregated_dict:
...
def metric_labels(
    metric_name, info_metrics=["reward", "los", "collisions", "velocity"]
):
    ...
    if metric_name.startswith("timers_"):
        y_label = (
            f"{metric_name.replace('timers_', '_')}".replace("_", " ").title() + " (s)"
        )
...
def plot_all_metrics(
    dictionary: Dict, output_dir, algo_colors: Dict[str, tuple], pdf=False, fill=True
):
    for metric, value in dictionary.items():
        ...
        if "env_id" in con_df.columns:
            fig, ax = plt.subplots(
                subplot_kw=dict(projection="polar"), figsize=FIGURE_SIZE
            )
    ...
    for idx, metric in enumerate(metric_list):
        ...
        for algo in sorted(list(con_df.algorithm.unique())):
            ...
            if "env_id" in _df.columns:
                ...
                #
                ax.plot(
                    E, metric_mean, label=algo.upper(), linewidth=2.0, color=algo_color
                )
                if fill:
                    lower_fill = np.clip(
                        metric_mean - metric_std, a_min=min_limit, a_max=None
...
def plot_report(
    ...
    new_dict = {
        metric: dfs for metric, dfs in dictionary.items() if metric in info_metrics
    }
...
def main():
    ...
    raw_dict = get_raw_dict_from_multirun_folder(multirun_folder=experiments_dir)
    ...
    for task in processed_data["urbanmarl"].keys():
        ...
        task_sample_efficiency_curves.figure.savefig(
            save_path, bbox_inches="tight", pad_inches=0.1
        )
    ...
    plot_catalogue(all_data, plot_dir, algo_colors, figsize=FIGURE_SIZE, pdf=False)
    ...
    plot_report(all_data, plot_dir, algo_colors, figsize=FIGURE_SIZE, pdf=True)
