...
def generate_mp4_from_pngs(image_dir: str, output_path: str, fps: int):
    """
    Collects PNG images, sorts them numerically, determines the maximum
    dimensions across all images, ensures they are even, and compiles
    them into an MP4 video.
    """
    ...
    # 1. First pass: Find the maximum width and height among all images
    print("Scanning images to determine the largest dimensions...")
    max_w, max_h = 0, 0
    for image_path in image_files:
        # Using PIL.Image.open is much faster for checking sizes 
        # than loading the entire array with imageio.imread
    ...
    # 2. Force the target dimensions to be even numbers (required by x264 codec)
    ...
    with imageio.get_writer(output_path, fps=fps, macro_block_size=None) as writer:
        for image_path in image_files:
            ...
            # 3. Check if the current image matches the target even dimensions
            if img.shape[:2] != (target_h, target_w):
                # Resize the image using Pillow to ensure uniformity
                img_pil = Image.fromarray(img)
                # Note: target_size in PIL is (width, height)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compile a sequence of PNG images into an MP4 video.")
