...
class VectorizedUrbanMap:
    ...
    def _build_ITU_P1410_maps(self, num_urbans: int = 72, seed: int = 0):
        ...
        h_maps_batch = np.zeros(
            (self.batch_size, self.sim_x, self.sim_y), dtype=np.float32
        )
        ...
        for env_idx in range(self.batch_size):
            ...
            if total_slots > 0:
                ...
                h_maps_batch[
                    env_idx, self.map_margin : end_x, self.map_margin : end_y
                ] = grid_2d[:len_x, :len_y]
                ...
                faces = self.create_building_faces(
                    X_flat, Y_flat, Z_flat, W_flat, W_flat, H_flat
                )
                self.building_faces.append(faces.tolist())

                # Fast Python list comprehension for data storage
                b_data = [
                    {"position": [x.item(), y.item(), 0], "size": [w, w, h.item()]}
        ...
        self.height_maps = torch.tensor(
            h_maps_batch, dtype=torch.float32, device=self.device
        )
    ...
    def reset(self, return_urban_info: bool = False):
        ...
        if self.info is None:
            self.info = torch.zeros(
                (self.batch_size, 7),  # alpha, beta, gamma, E, n_buildings, w, s
                dtype=torch.float32,
                device=self.device,
            )
        ...
        if return_urban_info:
            return {
                label: self.info[:, i] for i, label in enumerate(self.get_info_labels())
    ...
    def reset_at(self, env_idx: int, return_urban_info: bool = False):
        ...
        if total_slots > 0:
            ...
            # Assign the cleanly sliced grid
            h_map[self.map_margin : end_x, self.map_margin : end_y] = grid_2d[
                :len_x, :len_y
            ]
        ...
        self.height_maps[env_idx] = torch.tensor(
            h_map, dtype=torch.float32, device=self.device
        )
        ...
        if return_urban_info:
            return {
                label: self.info[env_idx, i].item()
    ...
    def generate_batch_maps(
        ...
        if self.info is None:
            self.info = torch.zeros(
                (self.batch_size, 7), dtype=torch.float32, device=self.device
            )
        ...
        if beta is None:
            ...
            beta = torch.randint(
                100, 750, (self.batch_size,), device=self.device
            ).float()
        if gamma is None:
        ...
        for b in range(self.batch_size):
            ...
            self.height_maps[b] = torch.tensor(
                h_map, dtype=torch.float32, device=self.device
            )
            self.info[b] = torch.tensor(
                [
                    alpha_np[b].item(),
                    beta_np[b].item(),
                    gamma_np[b].item(),
    ...
    def check_los_batch(
        ...
        max_step_length: Optional float. If provided, dynamically scales n_steps based on max ray distance.
        ...
        eff_steps = n_steps
        if max_step_length is not None and max_step_length > 0:
            max_dist = (
                torch.norm(p1.unsqueeze(2) - p2.unsqueeze(1), dim=-1).max().item()
            )
            eff_steps = max(n_steps, min(int(max_dist / max_step_length), 64))
        ...
        t = torch.linspace(0, 1, steps=eff_steps, device=self.device).view(
            1, 1, 1, eff_steps, 1
        )
        # Segments: shape (B, N, M, eff_steps, 3)
        ...
        grid_x = torch.clamp(
            ((segments[..., 0] - self.x_min)).long(), 0, self.sim_x - 1
        )
        grid_y = torch.clamp(
            ((segments[..., 1] - self.y_min)).long(), 0, self.sim_y - 1
        )
        seg_z = segments[..., 2]
    ...
    def check_collision_batch(
        ...
        """
        los = self.check_los_batch(p1, p2, n_steps, max_step_length)
        diag = torch.diagonal(los, dim1=1, dim2=2)
        collisions = (~diag).unsqueeze(-1)
        return collisions
    ...
    def gen_pos(
        ...
        if outdoor:
            result = self.gen_outdoor_pos(
                num_pos=num_pos, min_z=min_z, max_z=max_z, batch_idx=batch_idx
            )
            if normalized:
                result[..., 0] = (result[..., 0]) / (self.x_max - self.x_min)
                result[..., 1] = (result[..., 1]) / (self.y_max - self.y_min)
                result[..., 2] = (
                    (result[..., 2] - min_z) / (max_z - min_z)
                    if max_z > min_z
                    else max_z
                )
            return result
            ...
            if normalized:
                ...
                x = (
                    torch.rand((self.batch_size, num_pos), device=self.device)
                    * self.sim_x
                    + self.x_min
                )
                y = (
    ...
    def gen_outdoor_pos(
        ...
        if batch_idx is not None:
            ...
            z_column = min_z + torch.rand(size=(num_pos, 1), device=self.device) * (
                max_z - min_z
            )
            return torch.cat(
                [sampled_indices.view(num_pos, 2), z_column.view(num_pos, 1)],
                #  device=self.device,
                dim=-1,
            )
        else:
    ...
    def norm_pos(
        self, positions: torch.Tensor, min_z: float = None, max_z: float = None
    ) -> torch.Tensor:
    ...
    def denorm_pos(
        ...
        z = (
            positions[..., 2] * (max_z - min_z) + min_z
        )  # Assuming z is already in the correct scale
        return torch.stack([x, y, z], dim=-1)

    def pos_to_grid(self, positions: torch.Tensor) -> torch.Tensor:
        """Converts physical positions to grid indices for height map lookups."""
        grid_x = torch.clamp(
            ((positions[..., 0] - self.x_min)).long(), 0, self.sim_x - 1
        )
        grid_y = torch.clamp(
            ((positions[..., 1] - self.y_min)).long(), 0, self.sim_y - 1
        )
