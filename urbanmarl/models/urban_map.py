...
class VectorizedUrbanMap:
    """
    Tensor-accelerated ITU-R P.1410 procedural map handling generation 
    and fast Line-of-Sight (LoS) lookups across batch environments.
    """
    def __init__(
        self, 
        batch_size: int, 
        ...
        # Pre-allocate height maps for the batch: shape (batch_size, width, height)
        self.height_maps = torch.zeros((batch_size, self.sim_x, self.sim_y), 
                                       dtype=torch.float32, device=device)
        # info for procedural generation
        self.info = None
        # self.info = torch.zeros((batch_size, 7), # alpha, beta, gamma, E, n_buildings, w, s
        #                         dtype=torch.float32, device=device)
        self.has_envs = False
    ...
    def reset_at(self, env_idx: int, return_urban_info: bool = False):
        """
        Resets a specific environment index by regenerating its height map with new procedural parameters.
        Optionally returns the generated map information for analysis or observation purposes.
        """
        if self.info is None:
        ...
        total_slots = len(b_x) * len(b_y)
        if total_slots > 0:
            gamma = self.info[env_idx, 2].item()
            heights = np.random.rayleigh(gamma, size=total_slots)
            idx = 0
            for x in b_x:
                for y in b_y:
